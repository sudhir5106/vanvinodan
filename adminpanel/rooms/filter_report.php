...
$sql="SELECT Room_Id, Room_Name, rc.R_Category_Id, R_Category_Name, R_Capacity FROM tbl_room_master rm
	...
	if(empty($rs)==false)
		...
		while($val=mysql_fetch_array($rs)){ ?>
            <tr>
              ...
              <td><?php echo $val['R_Capacity'];?> Beds</td>
              <td><?php echo $val['R_Category_Name'];?></td>
              
              <td>
              	<a class="btn btn-success btn-xs" href="edit.php?id=<?php echo $val['Room_Id'];?>">Edit</a>
                <a class="btn btn-danger btn-xs delete" href="#" id="<?php echo $val['Room_Id'];?>">Delete</a>
              </td>
            </tr>
            <?php $i++;}
			}else{ ?>
            <td colspan="5" align="center">Opps No Data Found</td>
            <?php } ?>
          </tbody>
        </table>
